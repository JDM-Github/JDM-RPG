...
class JDMRootManager(ScreenManager):
    ...
    def update(self, dt: float):
        ...
        if self.__config.get("display_fps"):
            lapse = f"{(1 / self.elapseTime):.2f}"
            if float(lapse) < self.__lowest_lapse: self.__lowest_lapse = float(lapse)
            if App.get_running_app(): App.get_running_app().title = (
                App.get_running_app()._main_title + f" -> FPS: {lapse}")
    
    def on_stop(self):
        print(self.__lowest_lapse)
